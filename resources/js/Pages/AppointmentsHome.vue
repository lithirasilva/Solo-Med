<template>
    <div class="p-8 bg-gray-100 min-h-screen">
        ...
        <NextAppointment v-if="nextAppointment" :nextAppointment="nextAppointment" />


        <!-- Appointments Panel -->
        <div class="bg-white shadow-lg rounded-lg p-6">
            <!-- Tabs for Viewing Appointments -->
            <div class="flex border-b border-gray-200 mb-6">
                <button
                    @click="activeTab = 'upcoming'"
                    :class="{ 'border-b-2 border-orange-500': activeTab === 'upcoming' }"
                ...
                <button
                    ...
                    :class="{ 'border-b-2 border-orange-500': activeTab === 'past' }"
                    class="px-4 py-2 text-lg font-semibold focus:outline-none"
                >
                    Past Appointments
                </button>
            ...
            <div v-if="filteredAppointments.length > 0">
                <ul>
                    ...
                    >
                        <div>
                            <h3 class="text-xl font-semibold text-gray-800">Appointment ID: {{ appointment.appointmentId }}</h3>
                            <p class="text-xl font-semibold text-gray-800">{{ appointment.patient.firstName }} {{appointment.patient.lastName}}</p>
...
import NextAppointment from '@/Components/appointment_management/nextAppointment.vue';
...
const filteredAppointments = computed(() => {
    ...
    return props.appointments.filter(appointment => {
        ...
        const matchesSearchQuery = props.lockedPatient
            ? appointment.appointmentId.toString().includes(query) // Only search by Appointment ID
            : (
                appointment.patient.firstName.toLowerCase().includes(query) ||
                appointment.patient.email.toLowerCase().includes(query) ||
                appointment.patientId.toString().includes(query) // Include ID search as well
            );
